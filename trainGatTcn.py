...
def train(input_tensor,
          ...
          encoder_optimizer, 
          decoder_optimizer, 
          graph_attention_optimizer,
          long_temporal_optimizer,
          criterion, 
          max_length=30):
  ...
  graph_attention_optimizer.zero_grad()
  long_temporal_optimizer.zero_grad()
...
#  target_tensor = target_tensor[0]
  ...
  for ei in range(tcn_length - 30, tcn_length):
#    print("tcn_length:", tcn_length)
...
#      print("neighs:" , neigh_speed_tensor[:, 0])
      ...
      neighs_hidden.append(encoder_hidden.squeeze())   
    encoder_hidden = graph_attention(main_hidden, neighs_hidden)
#    print(input_speed.unsqueeze(1).shape)
#    print(encoder_hidden.shape, long_temporal(input_speed.unsqueeze(2))[:, -1, :].transpose(1, 0).shape)

#    encoder_hidden = encoder_hidden + long_temporal(input_speed.unsqueeze(2))[:, -1, :].unsqueeze(1).transpose(1, 0).float() #(N,C,L)
...
# Teacher forcing: Feed the target as the next input
  ...
  graph_attention_optimizer.step()
  long_temporal_optimizer.step()
#  print("raw_loss:", loss)
...
def test(input_tensor, 
  ...
  if model_type == "gru_gcn":
    ...
    for neigh_speed_tensor, neigh_time_tensor in zip(neighs_speed_tensor, neighs_time_tensor):      
      encoder_hidden = encoder.initHidden()
      for ei in range(input_length - 30, input_length):
        encoder_output, encoder_hidden = encoder(neigh_speed_tensor[:, ei + tcn_length - input_length], neigh_time_tensor[:, ei + tcn_length - input_length], encoder_hidden)
      neighs_hidden.append(encoder_hidden.squeeze())   
    encoder_hidden = graph_attention(main_hidden, neighs_hidden)
#    encoder_hidden = graph_attention.relu(graph_attention.fuse(encoder_hidden))
  if model_type == "gru_gcn_tcn":
...
#    encoder_hidden = encoder_hidden + long_temporal(input_speed.unsqueeze(1)) #(N,C,L)
...
def trainIters(encoder, 
  ...
  graph_attention_optimizer = optim.Adam(graph_attention.parameters(), lr=learning_rate)
  long_temporal_optimizer = optim.Adam(long_temporal.parameters(), lr=learning_rate)
...
for e in range(epoches): 
  ...
  for road in train_sets:
    loss = trainIters(encoder, decoder, graph_attention, batch_num, e, road)
    if loss > 0:
      loss_total += loss 
      loss_count += 1 
    batch_num += 1
    
  print("epoch:", e, "ave_test_loss:", loss_total / loss_count)
  ...
  torch.save(long_temporal, "/data/wuning/traffic-data/model/" + dataset + "/" + model_name + "/" + str(e) + ".t")   
